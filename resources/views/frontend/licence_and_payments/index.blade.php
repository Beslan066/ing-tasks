...
@section('content')
    @php
        ...
        $isPremium = isset($currentPlan) && $currentPlan === 'premium';
    @endphp
    ...
    <div id="license" class="container">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
                ...
                <div class="lg:col-span-2 backdrop-blur-md bg-black/30 rounded-lg shadow-sm md:shadow-md p-4 md:p-6">
                    ...
                    <div class="mt-4">
                        <div class="flex justify-between text-sm {{ $backgroundEnabled && $backgroundImage ? 'text-white/70' : 'text-gray-600' }} mb-1">
                            ...
                            <span>
                                @if($isPremium)
                                    {{ number_format($usedStorageGB ?? 0, 2) }} ГБ / {{ $maxStorageGB ?? 1024 }} ГБ
                                @else
                                    {{ number_format($usedStorageGB ?? 0, 2) }} ГБ / {{ $maxStorageGB ?? 2 }} ГБ
                                @endif
                            </span>
                        </div>
                        <div class="w-full {{ $backgroundEnabled && $backgroundImage ? 'bg-white/20' : 'bg-gray-200' }} rounded-full h-2">
                    ...
                    <div class="mt-5 pt-4 border-t border-white/20">
                        ...
                        <div class="flex flex-wrap gap-2">
                            ...
                            @else
                                ...
                                <span class="text-xs bg-white/10 text-white/80 px-2 py-1 rounded">✓ Файловое хранилище ({{ $isPremium ? '1 ТБ' : '2 ГБ' }})</span>
                            @endif
                        </div>
                    </div>
                </div>
            ...
            <!-- Действия с подпиской - для Премиум показываем только добавление пользователей -->
            @if($backgroundEnabled && $backgroundImage)
                <div class="backdrop-blur-md bg-black/30 rounded-lg shadow-sm md:shadow-md p-4 md:p-6 flex flex-col justify-between">
                    <div>
                        ...
                        <p class="text-white/70 text-sm mt-2">
                            @if($isPremium)
                                Добавьте дополнительных пользователей
                            @else
                                Обновитесь до Премиум или добавьте пользователей
                            @endif
                        </p>
                    </div>
        ...
        <!-- Сравнение тарифов - показываем ТОЛЬКО если не Премиум -->
        @if(!$isPremium)
            <div class="mb-8">
                @if($backgroundEnabled && $backgroundImage)
    ...
    <!-- МОДАЛ: Обновление до Премиум (только для базового тарифа) -->
    @if(!$isPremium)
