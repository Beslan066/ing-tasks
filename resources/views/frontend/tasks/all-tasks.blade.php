...
@section('content')
    ...
    <div class="">
        ...
        <div class="flex justify-between items-center mb-6">
            ...
            <div class="flex space-x-3">
                ...
                <a href="{{ route('welcome') }}"
                   class="  text-white px-4 py-2 rounded-lg transition flex items-center space-x-2" style="background: linear-gradient(180deg, #1a1f2e 0%, #161b28 100%);">
        ...
        <div class="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
            <div class="bg-white rounded-lg shadow p-4 text-center hover:shadow-lg transition cursor-pointer" onclick="filterByStatus('')">
                <div class="text-2xl font-bold text-blue-600">{{ $stats['total'] }}</div>
                <div class="text-sm text-gray-600">Всего задач</div>
            </div>
            <div class="bg-white rounded-lg shadow p-4 text-center hover:shadow-lg transition cursor-pointer" onclick="filterByStatus('назначена')">
                <div class="text-2xl font-bold text-purple-600">{{ $stats['new'] }}</div>
                <div class="text-sm text-gray-600">Новые</div>
            </div>
            <div class="bg-white rounded-lg shadow p-4 text-center hover:shadow-lg transition cursor-pointer" onclick="filterByStatus('в работе')">
                <div class="text-2xl font-bold text-orange-600">{{ $stats['in_progress'] }}</div>
                <div class="text-sm text-gray-600">В работе</div>
            </div>
            <div class="bg-white rounded-lg shadow p-4 text-center hover:shadow-lg transition cursor-pointer" onclick="filterByStatus('на проверке')">
                <div class="text-2xl font-bold text-yellow-600">{{ $stats['review'] }}</div>
                <div class="text-sm text-gray-600">На проверке</div>
            </div>
            <div class="bg-white rounded-lg shadow p-4 text-center hover:shadow-lg transition cursor-pointer" onclick="filterByStatus('выполнена')">
        ...
        <div id="filtersPanel" class="bg-white rounded-lg shadow mb-6 p-4 hidden transition-all duration-300">
            <div class="flex flex-wrap gap-4">
                ...
                <div class="flex items-end gap-2">
                    <button onclick="applyFilters()" class="bg-green-500 hover:bg-green-600 text-white px-6 py-2 rounded-lg transition">
                        <i class="fas fa-search mr-2"></i>Применить
                    </button>
                    <button onclick="resetFilters()" class="bg-gray-500 hover:bg-gray-600 text-white px-6 py-2 rounded-lg transition">
                ...
                <div class="bg-white rounded-lg shadow mb-6 p-4 ">
                    <div class="flex flex-wrap items-center justify-between gap-4">
                        ...
                        <div class="text-sm text-gray-500">
                            Показано: <span id="visibleCount">{{ $allTasks->count() }}</span> из <span id="totalCount">{{ $allTasks->total() }}</span> задач
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="bg-white rounded-lg shadow overflow-hidden">
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tr>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100" onclick="sortByColumn('name')">
                            Задача <i class="fas fa-sort ml-1"></i>
                        </th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100" onclick="sortByColumn('status')">
                            Статус <i class="fas fa-sort ml-1"></i>
                        </th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100" onclick="sortByColumn('priority')">
                            Приоритет <i class="fas fa-sort ml-1"></i>
                        </th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                            Отдел
                        </th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100" onclick="sortByColumn('deadline')">
                    ...
                    @foreach($allTasks as $task)
                        <tr class="task-row hover:bg-gray-50"
                            ...
                            data-department="{{ $task->department_id }}"
                            data-name="{{ $task->name }}"
                            ...
                            <td class="px-6 py-4">
                                <div class="text-sm font-medium text-gray-900">
                                    <a href="javascript:void(0)" onclick="openTaskViewModal({{ $task->id }})" class="hover:text-blue-600">
                                        {{ $task->name }}
                                    </a>
                                </div>
                            ...
                            <td class="px-6 py-4">
                                    <span class="px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full {{ $task->status === 'назначена' ? 'bg-blue-100 text-blue-800' : ($task->status === 'в работе' ? 'bg-purple-100 text-purple-800' : ($task->status === 'на проверке' ? 'bg-yellow-100 text-yellow-800' : ($task->status === 'выполнена' ? 'bg-green-100 text-green-800' : 'bg-red-400 text-white'))) }}">
                                        {{ $task->status }}
                                    </span>
                            </td>
                            <td class="px-6 py-4">
                                    <span class="px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full {{ $task->priority === 'низкий' ? 'bg-gray-100 text-gray-800' : ($task->priority === 'средний' ? 'bg-blue-100 text-blue-800' : ($task->priority === 'высокий' ? 'bg-orange-100 text-orange-800' : ($task->priority === 'критический' ? 'bg-red-100 text-red-800' : 'bg-gray-100 text-gray-800'))) }}">
                                        {{ $task->priority }}
                                    </span>
                            </td>
                            ...
                            <td class="px-6 py-4 text-sm text-gray-500">
                                @if($task->deadline)
                                    <span class="{{ $task->deadline->isPast() && $task->status !== 'выполнена' ? 'text-red-600 font-semibold' : '' }}">
